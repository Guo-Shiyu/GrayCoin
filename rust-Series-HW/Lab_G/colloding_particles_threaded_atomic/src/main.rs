use std::{
    borrow::BorrowMut,
    cell::{RefCell, RefMut},
    sync::{
        atomic::{AtomicUsize, Ordering},
        Arc, Mutex,
    },
};

#[derive(Debug)]
struct Particle {
    x: i32,
    y: i32,
}

impl Particle {
    const NUM_OF_THREADS: usize = 4;

    fn new(x: i32, y: i32) -> Self {
        Particle { x: x, y: y }
    }

    fn collide(&self, other: &Particle) -> bool {
        self.x == other.x && self.y == other.y
    }
}

impl Particle {
    fn rand_move(&mut self) {
        self.x = (rand::random::<f32>() * 10.0) as i32 % 10;
        self.y = (rand::random::<f32>() * 10.0) as i32 % 10;
    }
}

struct ParticleSystem {
    particles: Vec<Particle>,
    counter: AtomicUsize,
}

impl ParticleSystem {
    fn new() -> Self {
        ParticleSystem {
            particles: {
                let mut vec = Vec::new();
                for i in 0..10 {
                    for j in 0..10 {
                        vec.push(Particle::new(i, j));
                    }
                }
                vec
            },
            counter: AtomicUsize::new(0),
        }
    }
}

impl ParticleSystem {
    fn stir(&mut self) {
        for part in self.particles.iter_mut() {
            part.rand_move()
        }
    }
}

fn thread_main(list: &mut [Particle]) {
    for particle in list.iter_mut() {
        particle.rand_move()
    }

    // I guess enclosure_size is to be used as this
    // for i in 0..enclosure_size {
    //     list[i].rand_move()
    // }
}

fn collide_detect(system: &mut ParticleSystem) {
    for particle in system.particles.iter() {
        for other in system.particles.iter() {
            if particle.collide(other) {
                system.counter.fetch_add(1, Ordering::SeqCst);
            }
        }
    }
    println!("collide num:{}", system.counter.load(Ordering::SeqCst));
}

fn main() {
    println!("Hello, world!");

    let system = Arc::new(Mutex::new(ParticleSystem::new()));

    let thread_num = Particle::NUM_OF_THREADS as u32;
    let mut pool = scoped_threadpool::Pool::new(thread_num);
    let mut other_pool = scoped_threadpool::Pool::new(thread_num);

    let sys = system.clone();
    pool.scoped(|scope| {
        scope.execute(move || {
            let mut system = sys.lock().unwrap();
            thread_main(system.particles.as_mut_slice())
        });
    });

    other_pool.scoped(|scope| {
        scope.execute(move || {
            collide_detect(system.lock().unwrap().borrow_mut())
        })
    });
}
